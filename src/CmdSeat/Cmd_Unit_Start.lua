...
dps = "Calculating"
ttTenMil = 0
ttTenMilString = "Calculating"
calculating = false
lastDmgValue = shield.getShieldHitpoints()
adjustShield = false
...
function enemyDPS()
    ...
    local diff = lastDmgValue - newShield
    dpmTable[counter] = diff
    counter = counter + 1
    lastDmgValue = newShield
    ...
    if dpmTableLenght > 10 then
        ...
        if counter % 5 == 0 then
            ttTenMil = newShield / dps
            if ttTenMil < 60 then
                shieldDownColor = "red"
            elseif ttTenMil < 180 then
...
function drawEnemyDPS()
    ...
    if (calculating and shield.getState() == 1) or shield.isVenting() == 1 then
        enemyDPSHtml = [[
                    ...
                    <div class="enemyDPS">
                        <h3>Enemy-DPS: ]] .. dps .. [[</h3>
                        <h3 style="color:]] .. shieldDownColor .. [[">Shield Down in: ]] .. ttTenMilString .. [[</h3>
                        <table class="cd">
                            <tr>
...
function combineHudElements()
    ...
    alarmBorder()
    system.setScreen(alarmStyles ..
        cssAllyLocked ..
        fuelHtml .. brakeHtml .. alliesHtml ..
        threatsHtml .. ownInfoHtml .. speedHtml .. pipeInfoHtml .. enemyDPSHtml .. healthHtml)
end
